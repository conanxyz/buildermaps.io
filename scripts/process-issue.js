  ...
  changeLines.forEach(line => {
    ...
    if (match) {
      ...
      if (field === 'description') {
        ...
        changes.founded = newValue === 'null' || newValue === '' ? null : parseInt(newValue);
        ...
        changes.links.logo = logoValue.startsWith('/') ? `https://buildermaps.io${logoValue}` : logoValue;
...
function processIssue() {
  try {
    ...
    if (isAdd) {
      ...
      const sectorMatch = ISSUE_BODY.match(/Find the map file `public\/data\/maps\/([^`]+)\.json`/);
      ...
      if (sectorMatch && typeMatch) {
        ...
        console.warn('⚠️  Could not extract sector/type from issue body - map file not updated');
      }
      ...
      if (changes.category || changes.subcategory) {
        console.log('⚠️  Category/subcategory changes detected - manual review may be needed for map files');
      }
    }
